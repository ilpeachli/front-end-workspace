...
<head>
  ...
  <head>
    ### 외부방식 3. 링크 방식 ```html
    <head>
      <link rel="styleseet" type="text/css" href="ctyle.css" />
    </head>
    ```html ```css 선택자 { 속성: 값; 속성: 값; }
  </head>
</head>
````
...
- !important > 인라인방식 > 아이디 선택자 > 클래스 선택자 > 태그 선택자 > 전체 선택자
...
- 상대 크기
  ...
  - **rem** : 최상위 요소('html')에서 지정한 폰트의 크기를 1rem(16px)으로 계산
...
- 색상(hue)은 색상환으료 표시 (0~360, 360-빨간색, 120-초록색, 240-파란색)
...
- space-evely : 아이템들의 사이와 양 끝에 규일한 간격으로 정렬
...
- 여러 행 정렬
- `flex-wrap: warp;` 이 설정되 상태에서, 아이템들의 행이 2줄 이상인 경우 수직축 방향 정렬을 결정하는 속성
...
.item {
  flex: 1 1 auto;
  /* flex-grow: 1; flex-srink: 1: flex-basis: auto; */

  flex: 1 500px;
  /* flex-grow: 1; flex-srink: 1: flex-basis: 500px; */

  flex: 1;
  /* flex-grow: 1; flex-srink: 1: flex-basis: 0%; */
}
```
